...
var valid = {
  ...
  uniqueVisit : function (e, value) {
    ...
    let link = e.field("Patient")&&e.field("Patient").length>0?e.field("Patient")[0]:null;
    for (let ent=0; ent<entries.length; ent++) {
      if (fieldname.every(f=>{
        if (f.includes("Date")) 
          return my.gdate(entries[ent].field(f))==my.gdate(e.field(f));
        else if (f=="Patient") {
          let elink = entries[ent].field(f)&&entries[ent].field(f).length>0?entries[ent].field(f)[0]:null;
  ...
  createOp : function (e) {
    ...
    if(e.field("OpExtra") && e.field("Status") != "Not"){
      ...
      if(links.length>0){
        ...
        ent["Age"] =  Number(link.field("Age").replace(/\s*à¸›à¸µ/,""));
  ...
  updateOp : function (e) {
    ...
    if(old.field("OpExtra") == true && e.field("OpExtra") == true && old.field("Status") != "Not" && e.field("Status") != "Not"){
      ...
      if (s && link) {
        ...
        s.set("Age", Number(link.field("Age").replace(/\s*à¸›à¸µ/,"")));
      ...
      if(found.length>0) {
        ...
        for(let i=0; i<oss.length; i++) {
          if(old.field("PtName") && old.field("PtName") == oss[i].field("PtName") || old.field("HN") && old.field("HN") == oss[i].field("HN")) {
            ...
            oss[i].set("Age", Number(e.field("Age").replace(/\s*à¸›à¸µ/,"")));
